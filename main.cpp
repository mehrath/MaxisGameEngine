...
//    SDL_asprintf(&png_path, "%sSample.png", SDL_GetBasePath());  /* allocate a string of the full file path */
    SDL_asprintf(&png_path, "Sample.png");  /* allocate a string of the full file path */
...
SDL_AppResult SDL_AppIterate(void *appstate)
{
    ...
    const float x1 = 0.75f * WINDOW_WIDTH;
    const float y0 = 0.25f * WINDOW_HEIGHT;
    const float y1 = 0.75f * WINDOW_HEIGHT;
    ...
    for (i = 0; i < 8; i++) {
        ...
        const float y = (i & 2) ? -0.25f : 0.25f;
        const float z = (i & 4) ? -0.25f : 0.25f;
